  ...
  return (
    <>
      ...
      <section className="py-12 w-full bg-gradient-to-r from-fuchsia-200 to-violet-100 min-h-screen">
        ...
        <form action="https://formspree.io/f/mjkbewjr" method="post" className='w-full'>
            ...
            <div className='flex h-fit shadow-book_online bg-white rounded-lg col-span-2 mx-3 flex-col flex-wrap gap-6 px-5 py-6'>
              ...
              {step === 2 && (
                <>
                  <div className='flex flex-wrap flex-col gap-4'>
                    <div className='flex flex-wrap flex-col gap-2 '>
                      <label htmlFor="name" className='text-2xl font-medium pb-4'>
                        Step 5: Contact Information
                      </label>
                      <div className='grid grid-cols-1 lg:grid-cols-2 gap-3'>
                        <Input name="name" required placeholder="Full Name" value={formData.name} onChange={handleChange} />
                        <Input name="email" required placeholder="Email" value={formData.email} onChange={handleChange} />
                      </div>
                      <div>
                        <Input name="number" required placeholder="Number"  onChange={handleChange} />
                    ...
                    <div className='flex flex-wrap flex-col'>
                      ...
                      <p className='text-gray-700 pb-3 '>Select the date and time you would like us to arrive.  Once we receive your booking,
                        we'll match you with one of our top cleaning pros and we'll send you a confirmation.</p>
                      <Input name="date" required placeholder="Date" type="date" value={formData.date} onChange={handleChange} />
                    ...
                    <div lassName='flex flex-wrap flex-col gap-3'>
                      ...
                      <div className='grid grid-cols-2 lg:grid-cols-2 gap-4 pt-3'>
                        <Input name="address" required placeholder="Address" value={formData.address} onChange={handleChange} />
                        <Input name="postalCode" required placeholder="Postal Code" value={formData.postalCode} onChange={handleChange} />
                      </div>
                      <div className='flex flex-wrap flex-col pt-6 gap-4'>
                        <div className='flex flex-wrap flex-col gap-2'>
                          <label className='font-semibold text-xl'>Where can we park?</label>
                          <Select >
                            <SelectTrigger className="w-full">
                              <SelectValue placeholder="In my garage" />
                            </SelectTrigger>
                            <SelectContent name='where_to_park'>
                              <SelectItem value="garage">In my garage</SelectItem>
                              <SelectItem value="driveway">In my driveway</SelectItem>
                              <SelectItem value="street">On the street</SelectItem>
                              <SelectItem value="parkingLot">In a parking lot</SelectItem>
                              <SelectItem value="noParking">No dedicated parking</SelectItem>
                            </SelectContent>
                          </Select>
                        </div>
                        <div className='flex flex-wrap flex-col gap-2'>
                          <label className='font-semibold text-xl'>How will we gain access to your home?</label>
                          <Select >
                            <SelectTrigger className="w-full">
                              <SelectValue placeholder="I will be home" />
                            </SelectTrigger>
                            <SelectContent name='how_to_come_your_home'>
                              <SelectItem value="home">I will be home</SelectItem>
                              <SelectItem value="keyUnderMat">Key under the mat</SelectItem>
                              <SelectItem value="lockbox">Key in a lockbox</SelectItem>
                              <SelectItem value="neighbor">Neighbor has a key</SelectItem>
                              <SelectItem value="securityCode">Security code provided</SelectItem>
                            </SelectContent>
                          </Select>
                        </div>
                        <div className='flex flex-wrap flex-col gap-2'>
                          <label className='font-semibold text-xl'>Is there running water and electricity at the property?</label>
                          <Select>
                            <SelectTrigger className="w-full">
                              <SelectValue placeholder="Yes, both available" />
                            </SelectTrigger>
                            <SelectContent name='running_water'>
                              <SelectItem value="both">Yes, both available</SelectItem>
                              <SelectItem value="waterOnly">Only running water available</SelectItem>
                              <SelectItem value="electricityOnly">Only electricity available</SelectItem>
                              <SelectItem value="neither">Neither available</SelectItem>
                            </SelectContent>
                          </Select>
                        </div>
                        <div className='flex flex-wrap flex-col gap-2'>
                          <label className='font-semibold text-xl'>On a scale of 1-5, how clean would you estimate your home to be?</label>
                          <Select>
                            <SelectTrigger className="w-full">
              ...
              <div className='py-4 flex flex-wrap flex-col float-right w-full gap-4'>
                {step === 1 && (
                  <button type="button" onClick={goToNextStep} className='bg-[#007aff] py-3 float-right text-center justify-end w-fit capitalize flex items-center text-white gap-2 px-4 font-medium rounded-lg'>
                ...
                {step === 2 && (
                  <>
                    <div className='flex flex-wrap justify-between flex-4 gap-4 w-full'>
                      <button type="button" onClick={goToPreviousStep} className='px-5 py-3 flex items-center bg-gray-200 text-gray-700 rounded-lg'>
                        <MdArrowBackIos className='text-lg' />Back
                      </button>
                      <button type="submit" className='w-fit bg-[#007aff] py-3 float-right text-center justify-center capitalize flex items-center text-white gap-1 px-4 font-medium rounded-lg'>
